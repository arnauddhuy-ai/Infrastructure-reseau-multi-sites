...
Pour tester et vérifier l'état du réseau :
Commande	Action
...
Paramètre	Valeur
Sauvegarde officielle	copy running-config startup-config
Méthode abrégée	write memory (ou wr)
10.3 Retour en arrière

Pour l'annulation et la réinitialisation :
Paramètre	Valeur
Supprimer une config	no <commande>
...
Conclusion générale Ce projet illustre concrètement le fonctionnement d’un réseau professionnel moderne et opérationnel. Il constitue une base solide pour comprendre et administrer des infrastructures réseau complexes en environnement réel.
